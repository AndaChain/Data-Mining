...
class model:
	def __init__( self, learning_rate=0.01, iterations=400, penality=1 ):
	...
	def training(self, X, Y):
		...
		self.rss_ = -( 2*( np.sum((Y_pred - Y) ** 2) ) )+ (self.penality * self.W ** 2)
		self.rss_min = self.rss_
		self.rss_hist = [  self.rss_  ]
	...
	def update_weights(self):
		...
		# calculate gradients
		rss = -( 2*( np.sum((Y_pred - Y) ** 2) ) )+ (self.penality * self.W ** 2)
		#rss = ( - ( 2 * ( self.X.T ).dot( self.Y - Y_pred ) ) + ( 2 * self.penality * self.W ) )
		
		# print(self.W, self.b, self.rss_min, rss) # [0.97061375] 0.2521666672574807 [-432.64450247] [-432.64450247]
		#if(self.rss_min > rss):
			#return self
...
if __name__ == '__main__':
	...
	for al in alpha:
		...
		index = np.where(-1*ridge.rss_hist == min(-1*ridge.rss_hist))
		plt.scatter(ridge.W_hist[index[0]], -1*ridge.rss_hist[index[0]])
		print(ridge.W_hist[index[0]], -1*ridge.rss_hist[index[0]])
		
		plt.plot(ridge.W_hist, -1*ridge.rss_hist, label=f"alpha={al}")
		plt.legend()
	#plt.xlim(min(ridge.W_hist),max(ridge.W_hist))
	#plt.ylim(min(-1*ridge.rss_hist),max(-1*ridge.rss_hist))
	plt.show()
